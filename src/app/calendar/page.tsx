...
import { Calendar, MapPin, Clock, ExternalLink, CalendarDays, Sparkles } from 'lucide-react';
...
export default async function PublicCalendarPage({ searchParams }: CalendarPageProps) {
  ...
  try {
    const [b, evs] = await Promise.all([
      ...
      getSchoolEvents(category),
    ]);
    branding = b;
    events = evs;
  } catch (err) {
    console.warn('Error fetching calendar in PublicCalendarPage:', err);
    events = await getSchoolEvents(category);
  }
    ...
    <div className="min-h-screen flex flex-col bg-slate-50 text-slate-900 font-sans">
      ...
      <main className="flex-1 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 space-y-6 sm:space-y-8 w-full">
        ...
        <div className="text-center max-w-xl mx-auto space-y-2">
          <span className="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full text-xs font-bold bg-blue-100 text-blue-800">
            <Sparkles className="w-3.5 h-3.5 text-blue-600" />
            School Calendar
          </span>
          <h1 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-slate-900 tracking-tight" style={{ fontFamily: 'var(--font-heading)' }}>
                  ...
                  <div className="space-y-1.5 min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-4 text-xs text-slate-500 pt-0.5 flex-wrap">
                      ...
                      <div className="flex items-center gap-1 text-slate-400">
                        ...
                        <span>{ev.timeRange || '08:30 - 16:30 น.'}</span>
                      </div>
                    </div>
                  </div>
                </div>
